...
export async function allTaskDueToday(params, signal) {
  const url = new URL(`${API_BASE_URL}/tasks`);
  return await fetchJson(url, {signal }, [])
}
export async function getAllHouseholdMember(params, signal) {
  const url = new URL(`${API_BASE_URL}/household-members`);
  return await fetchJson(url, {signal }, [])
}
